...
[상장 국가 국기 및 중국 본토 상장 종목 표시 개선 업데이트]

티커 옆 상장 국가 국기 표시: 테이블의 티커 뱃지 오른쪽에 해당 종목이 상장된 국가의 국기 이모지를 함께 노출하도록 개선했습니다.

중국 본토 상장 종목 추가: 기존 중국계 기업 중 일부는 미국 ADR 또는 OTC 티커로 내려와 미국 국기가 표시될 수 있었습니다. 중국 국기가 실제로 노출되도록 상하이·선전 거래소 상장 대형주를 테이블 데이터 후보에 추가했습니다.

- 예: `601939.SS`, `601398.SS`, `600519.SS`, `300750.SZ` 등은 중국 국기와 함께 표시됩니다.
- `BABA`, `PDD`, `TCEHY`처럼 미국 거래소 또는 미국 OTC에 상장된 티커는 상장지 기준으로 미국 국기가 표시됩니다.

비달러 시가총액 환산 개선: 중국 위안(CNY)과 홍콩달러(HKD) 환율을 함께 조회하여, 중국·홍콩 상장 종목의 시가총액 순위가 달러 기준으로 더 정확하게 계산됩니다.

현지 통화 가격 표시 개선: 주가 표시는 각 종목의 거래 통화에 맞춰 표시되며, 중국 본토 종목은 `CN¥` 형식으로 확인할 수 있습니다.
